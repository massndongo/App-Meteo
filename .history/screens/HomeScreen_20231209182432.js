...
export default function HomeScreen() {
  ...
  const handleSearch = search=>{
    if(search)
      fetchLocations({
        ...
        // console.log("tessssssssssssssssssssssssssssssssssssssssssst");
        console.log(data);
        setLocations(data);
      })
  }
  ...
  const handleLocation = (latitude, longitude) => {
    ...
    fetchWeatherForecast({
      ...
      setLoading(false);    if (data && data.forecast && data.current) {
      setLoading(false);
      setWeather({
        location: {
          name: data.location.name,
          sys: {
            country: data.location.sys.country
          }
        },
        current: {
          temp_c: data.current.temp_c,
          wind: {
            deg: data.current.wind.deg,
      ...
      storeData('coord', `${latitude},${longitude}`);
    } else {
          ...
          loading? (
            ...
            <SafeAreaView className="flex flex-1">
                          ...
                          return (
                            <TouchableOpacity 
                              ...
                              className={"flex-row items-center border-0 p-3 px-4 mb-1 "+borderClass}>
                                ...
                                <Text className="text-black text-lg ml-2">{loc?.name}, {loc?.sys.country}</Text>
                            </TouchableOpacity>
                          )
                        })
                      }
              ...
              <View className="mx-4 flex justify-around flex-1 mb-2">
                {/* location */}
                <Text className="text-white text-center text-2xl font-bold"> gjdh
                  {location?.name}, <Text className="text-lg font-semibold text-gray-300">{location?.sys.country}</Text>
                </Text>
                {/* weather icon */}
                ...
                <View className="space-y-2">
                    ...
                    <Text className="text-center text-white text-xl tracking-widest">{current?.wind?.deg}</Text>
                </View>
