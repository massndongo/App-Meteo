              ...
              <View className="mb-2 mx-6 flex-row justify-between items-center">
                {
                  weather?.list.map((item,index)=>{
                    ...
                    dayName = dayName.split('T')[0];

                    return (
                      <View key={index} className="flex justify-center items-center rounded-3xl py-3 space-y-1 m-2" 
                        style={{backgroundColor: theme.bgWhite(0.15), width: '29%'}}>
                        <Image 
                          // source={{uri: 'https:'+item?.day?.condition?.icon}}
                          source={weatherImages[item?.weather[0].main || 'other']}
                            className="w-11 h-11" />
